...
import EmployeeItemService, {handleDownload} from 'services/EmployeeItemService';
...
const showConfirm = (rowID) => {
    ...
    confirm({
        ...
        cancelText: 'Cancelar',
        async onOk() {
            const response = await EmployeeItemService.delete(rowID)
...
export const DefaultEmployeeEmail = () => {
    ...
    useEffect(() => {
        EmployeeItemService.list().then(result => setDataResult(result))
    }, []);
