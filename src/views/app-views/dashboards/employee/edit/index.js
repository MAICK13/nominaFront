...
export const EditEmployee = () => {
    ...
    useEffect(() => {
        EmployeeService.get_employee(params.id).then(result => {
            form.setFieldsValue({
                employee_name: result.employee_name,
                employee_second_name: result.employee_second_name,
                employee_other_name: result.employee_other_name,
                employee_last_name: result.employee_last_name,
                employee_second_last_name: result.employee_second_last_name,
                employee_last_name_married: result.employee_last_name_married,
                employee_gender: result.employee_gender,
                employee_civil_state: result.employee_civil_state,
                employee_born_date: result.employee_born_date,
                employee_born_place: result.employee_born_place,
                employee_dpi: result.employee_dpi,
                employee_start_date: result.employee_start_date,
                employee_contract: result.employee_contract,
                job: ''+result.job
            })
        })
    }, []);
    ...
    const onFinish = async values => {
        const response = await EmployeeService.edit(values, params.id)
    ...
    return <>
        <div style={{display:'flex', width:'100%', justifyContent:'space-between', marginBottom:'30px'}}>
            <div>
                <h2>Editar Registro de Colaboradores</h2>
                <Breadcrumb>
                    <Breadcrumb.Item>
                        <a href="../list">Registro de colaborares</a>
                    </Breadcrumb.Item>
